...
from django.db.models import QuerySet, Q,Max,F, ExpressionWrapper, DateField,IntegerField, Subquery, Exists
...
from risk.api.serializers.annual_warned_risk_partners_serializers import *
...
class AnnualWarnedRiskPartnerList(ModelViewSet, QueryListAPIView):
    serializer_class = AnnualWarnedRiskPartnerListSerializer
    filterset_class = AnnualWarnedRiskPartnerFilter
    ...
    def get_queryset(self):
        ...
        contracts_with_multiple_notices = Lease.objects.filter(
            contract__partner=OuterRef('pk')
        ).annotate(
            notice_count=Count(
        ...
        queryset = Partner.objects.select_related(*custom_related_fields).prefetch_related(*prefetch_related_fields).filter(
            ...
            #         Q(partner_contracts__contract_warning_notices__service_date__gte=now()-timedelta(days=365)) &

...
def export_to_terminated_risk_partners_for_sms(self):
    ...
    base_path = os.path.join(os.getcwd(), "media", "docs", str(self.user.user_companies.filter(is_active=True).first().company.uuid), "risk", "to_terminated_risk_partners", "documents")
    ...
    excel_dosyasi_adi = f"{base_path}/{datetime.today().strftime('%d-%m-%Y')}-fesih-edilecekler-sms.xlsx"

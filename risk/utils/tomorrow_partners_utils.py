...
def export_tomorrow_partners(self):
    ...
    objs = Lease.objects.select_related("contract","contract__partner","contract__quotation_obj__quick_quotation").filter(
        ...
        ~Q(lease_installments__type__in=['4','5']) 
    ...
    base_path = os.path.join(os.getcwd(), "media", "docs", str(self.user.user_companies.filter(is_active=True).first().company.uuid), "risk", "tomorrow_partners", "documents")

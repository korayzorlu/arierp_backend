...
def export_risk_partners_for_sms(self):
    ...
    base_path = os.path.join(os.getcwd(), "media", "docs", str(self.user.user_companies.filter(is_active=True).first().company.uuid), "risk", "risk_partners", "documents")
    ...
    excel_dosyasi_adi = f"{base_path}/{datetime.today().strftime('%d-%m-%Y')}-risk-durumunda-olanlar-sms.xlsx"
...
def export_overdue_leases(self):
    ...
    base_path = os.path.join(os.getcwd(), "media", "docs", str(self.user.user_companies.filter(is_active=True).first().company.uuid), "risk", "overdue_leases", "documents")
    if not os.path.exists(base_path):
            os.makedirs(base_path)
    ...
    excel_dosyasi_adi = f"{base_path}/{datetime.today().strftime('%d-%m-%Y')}-vadesi-gecmisler-ham-data.xlsx"
    with pd.ExcelWriter(excel_dosyasi_adi, engine='openpyxl') as writer:
            df.to_excel(writer, sheet_name='Sayfa', index=False)

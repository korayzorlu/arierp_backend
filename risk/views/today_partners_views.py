...
class ExportTodayPartnersView(LoginRequiredMixin,View):
    def post(self, request, *args, **kwargs):
        ...
        exporter = BaseExporter(
            ...
            export_url="/risk/today_partners_excel",
...
class TodayPartnersExcelView(LoginRequiredMixin,View):
    def get(self, request, *args, **kwargs):
        file_path = os.path.join(settings.BASE_DIR, "media", "docs", str(self.request.user.user_companies.filter(is_active = True).first().company.uuid), "risk", "today_partners", "documents",f"{datetime.today().strftime('%d-%m-%Y')}-bugün-ödemesi-olanlar.xlsx")

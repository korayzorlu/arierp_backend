...
class ExportTomorrowPartnersView(LoginRequiredMixin,View):
    def post(self, request, *args, **kwargs):
        ...
        exporter = BaseExporter(
            ...
            export_url="/risk/tomorrow_partners_excel",
...
class TomorrowPartnersExcelView(LoginRequiredMixin,View):
    def get(self, request, *args, **kwargs):
        file_path = os.path.join(settings.BASE_DIR, "media", "docs", str(self.request.user.user_companies.filter(is_active = True).first().company.uuid), "risk", "tomorrow_partners", "documents",f"{datetime.today().strftime('%d-%m-%Y')}-yarın-ödemesi-olanlar.xlsx")

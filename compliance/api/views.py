...
class BlackListPersonList(ModelViewSet, QueryListAPIView):
    serializer_class = BlackListPersonListSerializer
    filterset_class = BlackListPersonFilter
    ...
    def get_queryset(self):
        ...
        queryset = BlackListPerson.objects.select_related(*custom_related_fields).filter(
            Q(company = active_company.company if active_company else None)
        ).order_by(ordering if ordering else '-created_date')

        query = self.request.query_params.get('search[value]', None)
        if query:
            search_fields = ["name","tc_vkn_passport_no"]
...
class PepPartnerList(ModelViewSet, QueryListAPIView):
    serializer_class = PepPartnerListSerializer
    filterset_class = PepPartnerFilter
    filter_backends = [OrderingFilter,DjangoFilterBackend]
    ordering_fields = ['name','tc_vkn_no','crm_code','birthday','sgk_job_code','is_pep','pep_degree','pep_description']
    ordering = ['crm_code']
    # pagination_class = DatatablesPagination
    def get_pagination_class(self):
        paginate = self.request.query_params.get('paginate')
    ...
    def get_queryset(self):
        ...
        query = self.request.query_params.get('search[value]', None)
        if query:
            search_fields = ["name","tc_vkn_no","crm_code","birthday","sgk_job_code","pep_degree","pep_description"]
...
class ThirdPersonList(ModelViewSet, QueryListAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = ThirdPerson.objects.select_related(*custom_related_fields).prefetch_related(*prefetch_related_fields).filter(
            Q(company=active_company.company if active_company else None)
        )

        query = self.request.query_params.get('search[value]', None)
        if query:
            search_fields = ["name","tc_vkn_no"]
...
class VPosThirdPersonList(ModelViewSet, QueryListAPIView):
    serializer_class = ThirdPersonListSerializer
    filterset_class = ThirdPersonFilter
    filter_backends = [OrderingFilter,DjangoFilterBackend]
    ordering_fields = ['created_date','name','tc_vkn_no']

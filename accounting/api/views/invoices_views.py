...
from ..serializers.invoices_serializers import *
from ..filters.invoices_filters import *
...
class InvoiceList(ModelViewSet, QueryListAPIView):
    ...
    #ordering_fields = list(Invoice._meta.get_fields()) + ['total_tl']
    ordering_fields = [f.name for f in Invoice._meta.get_fields() if hasattr(f, 'name')]
    ordering = ['-date']
    ...
    def get_queryset(self):
        ...
        custom_related_fields = ["company","partner","lease"]
        
        queryset = Invoice.objects.select_related(*custom_related_fields).filter(
            Q(company=active_company.company if active_company else None)
        )

        query = self.request.query_params.get('search[value]', None)
        if query:
            search_fields = ["lease__code","partner__name","invoice_no","type","date"]

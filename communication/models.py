...
class WhatsAppContact(models.Model):
    ...
    def __str__(self):
        return str(f"{self.phone_number} - {self.name}")

class WhatsAppMessage(models.Model):
    uuid = models.UUIDField(default=uuid.uuid4, unique=True)
    company = models.ForeignKey(Company, on_delete=models.CASCADE, related_name="whats_app_messages")
    
    contact = models.ForeignKey(WhatsAppContact, on_delete=models.CASCADE, related_name="contact_whatsapp_messages")
    ...
    def __str__(self):
        return str(f"{self.contact.phone_number} - {self.contact.name} - {self.wa_message_id}")
...
class PartnerScoreList(ModelViewSet, QueryListAPIView):
    serializer_class = PartnerScoreListSerializer
    filterset_class = PartnerScoreFilter
    filter_backends = [OrderingFilter,DjangoFilterBackend]
    ordering_fields = '__all__'
    ordering = ['crm_code_int']
    # pagination_class = DatatablesPagination
    ...
    def get_queryset(self):
        ...
        queryset = PartnerScore.objects.select_related(*custom_related_fields).filter(
            Q(company = active_company.company if active_company else None)
        ).annotate(
            crm_code_int=Cast('partner__crm_code', IntegerField())
        ).exclude(

...
from .utils.lease_utils import fetch_leases_from_leaseflex,fetch_exchanged_amounts_utils,fetch_tufe_exchanged_amounts_utils,get_lease_delay,get_faulty_lease,fetch_leases_from_ifs,fetch_purchase_payments_from_ifs,get_leases_excel_file
...
@shared_task()
def fetch_tufe_exchanged_amounts(company):
    fetch_tufe_exchanged_amounts_utils(company)
...
@shared_task()
def fetch_kdv_leases(company):
    excel_file = pd.ExcelFile("files/kdv-listesi.xlsx")
    sheet_name = excel_file.sheet_names[0]

    file_data = pd.read_excel("files/kdv-listesi.xlsx", sheet_name)
    df = pd.DataFrame(file_data)

    leases = Lease.objects.select_related().all()
    leases.update(is_kdv_diff = False)
...
@shared_task()
def get_lease_delay_task(company,lease_id):
    get_lease_delay(company, lease_id)

@shared_task()
def get_faulty_lease_task(company):
    get_faulty_lease(company)

@shared_task()
def get_leases_excel(company):
    get_leases_excel_file(company)
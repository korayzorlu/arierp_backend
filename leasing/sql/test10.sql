SELECT
    ...
    CASE
        WHEN (
            ...
            WHERE SOURCE_LOP_ID = lop.SourceLOPId AND ISNULL(IS_DELETED, 0) = 0
        ) >= 1 THEN 'Evet'
        ELSE 'Hayır'
    END AS Corporates,
    (
        ...
        WHERE QUO_HEADER_ID = lop.ApplicationID AND ISNULL(IS_DELETED, 0) = 0
    ) AS BBSN_NO
FROM
    ...
    LEFT OUTER JOIN dbo.QuotationProjectVendorType qvt (NOLOCK) ON qvt.VendorTypeId = lop.VendorTypeId
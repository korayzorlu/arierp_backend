...
class ExportPurchasePaymentsView(LoginRequiredMixin,View):
    def post(self, request, *args, **kwargs):
        ...
        exporter = BaseExporter(
            ...
            export_url="/purchasing/purchase_payments_excel",
...
class PurchasePaymentsExcelView(LoginRequiredMixin,View):
    def get(self, request, *args, **kwargs):
        file_path = os.path.join(settings.BASE_DIR, "media", "docs", str(self.request.user.user_companies.filter(is_active = True).first().company.uuid), "purchasing", "purchase_payments", "documents",f"{datetime.today().strftime('%d-%m-%Y')}-satici-odemeleri.xlsx")

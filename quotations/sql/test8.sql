...
FROM dbo.RPR_QUO quo
    LEFT JOIN dbo.CrmCustomerListCombo  crmInst   ON quo.CRM_CUSTOMER_ID              = crmInst.CustomerId
    LEFT JOIN dbo.RPR_PARAMETER         custType  ON quo.CUSTOMER_TYPE_ID              = custType.PARAMETER_VALUE
                                                 AND custType.PARAMETER_NAME           = 'CUSTOMER_TYPE_ID'
    LEFT JOIN dbo.RPR_QUO_ITEM          quoItem   ON quo.RPR_QUO_ID                   = quoItem.RPR_QUO_ID
    LEFT JOIN dbo.RPR_PROJECT           project   ON quoItem.PROJECT_ID                = project.PROJECT_ID
    LEFT JOIN dbo.RPR_PROJECT_BLOCK     block     ON quoItem.BLOCK_ID                  = block.BLOCK_ID
    LEFT JOIN dbo.RPR_PROJECT_FREE_PART freePart  ON quoItem.FREE_PART_ID              = freePart.FREE_PART_ID
    LEFT JOIN dbo.RPR_QUO_PAYMENT_PLAN  qpp       ON quo.RPR_QUO_ID                   = qpp.RPR_QUO_ID
    LEFT JOIN dbo.GeneralCurrency       grossCurr ON quoItem.GROSS_SALE_AMOUNT_CURR_ID = grossCurr.CurrencyId
    LEFT JOIN FoundationStatuteMenu     subStat   ON quo.LAST_SUB_STATU_ID             = subStat.DefinitionId
    -- LEFT JOIN dbo.GeneralTax         tax       ON tax.TaxGroupId                   = quoItem.TAX_GROUP_ID

from django.shortcuts import render
from django.contrib.auth.mixins import LoginRequiredMixin
from django.views import View
from django.http import JsonResponse, FileResponse, HttpResponse
from django.core.files.storage import default_storage
from django.core.files.base import ContentFile
from django.utils.crypto import get_random_string
from django.conf import settings
from channels.layers import get_channel_layer
from asgiref.sync import async_to_sync

from utils.mixins import CompanyOwnershipRequiredMixin

from quotations.models import *
from quotations.utils import is_valid_quick_quotation_data
from common.models import ImportProcess
from common.utils.import_utils import BaseImporter
from common.utils.websocket_utils import send_alert
from partners.models import Partner
from contracts.models import Contract

import os
import json
import pandas as pd
from decimal import Decimal

# Create your views here.

class AddQuickQuotationView(LoginRequiredMixin,View):
    def post(self, request, *args, **kwargs):
        data = json.loads(request.body)

        if not request.user.is_authenticated:
            return JsonResponse({'message': 'Auth failed!.','status':'error'}, status=401)
        
        valid, response = is_valid_quick_quotation_data(data)
        if not valid:
            return response
        
        company = Company.objects.filter(id = data.get('companyId')).first()
        active_company = request.user.user_companies.filter(is_active = True, company = company).first()

        if not company or not active_company:
            return JsonResponse({'message': 'Sorry, something went wrong!','status':'error'}, status=400)
        
        partner = Partner.objects.filter(uuid = data.get('partner')).first()
        currency = Currency.objects.filter(uuid = data.get('currency')).first()
        status = Status.objects.filter(uuid = data.get('status')).first()
        currency = Currency.objects.filter(uuid = data.get('currency')).first()

        obj = QuickQuotation.objects.create(
            company = company,
            code = data.get('code'),
            partner = partner,
            status = status,
            customer_type = data.get('customer_type'),
            project = data.get('project'),
            block = data.get('block'),
            unit = data.get('unit'),
            currency = currency,
            vat = Decimal(str(data.get('vat'))),
            price = Decimal(str(data.get('price'))),
            customer_signature_date = data.get('customer_signature_date'),
            unit_delivery_date = data.get('unit_delivery_date'),
            is_tufe = data.get('is_tufe'),
            ortalama_tahsil_suresi = data.get('ortalama_tahsil_suresi'),
            devremulk = data.get('devremulk'),
            start_date = data.get('unit'),
            finish_date = data.get('unit'),
            bbsn = data.get('unit'),
        )
        obj.save()

        return JsonResponse({'message': 'Created successfully!','status':'success'}, status=200)
    
class UpdateQuickQuotationView(LoginRequiredMixin,CompanyOwnershipRequiredMixin,View):
    model = QuickQuotation
    
    def post(self, request, *args, **kwargs):
        data = json.loads(request.body)

        valid, response = is_valid_quick_quotation_data(data)
        if not valid:
            return response

        partner = Partner.objects.filter(uuid = data.get('partner')).first()
        currency = Currency.objects.filter(uuid = data.get('currency')).first()
        status = Status.objects.filter(uuid = data.get('status')).first()
        currency = Currency.objects.filter(uuid = data.get('currency')).first()

        obj = QuickQuotation.objects.filter(uuid = data.get('uuid')).first()
        obj.code = data.get('code')
        obj.partner = partner
        obj.status = status
        obj.customer_type = data.get('customer_type')
        obj.project = data.get('project')
        obj.block = data.get('block')
        obj.unit = data.get('unit')
        obj.currency = currency
        obj.vat = Decimal(str(data.get('vat')))
        obj.price = Decimal(str(data.get('price')))
        obj.customer_signature_date = data.get('customer_signature_date')
        obj.unit_delivery_date = data.get('unit_delivery_date')
        obj.is_tufe = data.get('is_tufe')
        obj.ortalama_tahsil_suresi = data.get('ortalama_tahsil_suresi')
        obj.devremulk = data.get('devremulk')
        obj.start_date = data.get('unit')
        obj.finish_date = data.get('unit')
        obj.bbsn = data.get('unit')
        obj.save()

        return JsonResponse({'message': 'Saved successfully!','status':'success'}, status=200)
    
class DeleteQuickQuotationView(LoginRequiredMixin,CompanyOwnershipRequiredMixin,View):
    model = QuickQuotation

    def post(self, request, *args, **kwargs):
        data = json.loads(request.body)

        obj = QuickQuotation.objects.filter(uuid = data.get('uuid')).first()
        obj.delete()

        return JsonResponse({'message': 'Removed successfully!','status':'success'}, status=200)
    
class DeleteQuickQuotationsView(LoginRequiredMixin,CompanyOwnershipRequiredMixin,View):
    model = QuickQuotation

    def post(self, request, *args, **kwargs):
        data = json.loads(request.body)
        uuids = data.get('uuids')

        for uuid in uuids:
            obj = QuickQuotation.objects.filter(uuid = uuid).first()
            obj.delete()

        return JsonResponse({'message': 'Removed successfully!','status':'success'}, status=200)
    
class DeleteAllQuickQuotationsView(LoginRequiredMixin,CompanyOwnershipRequiredMixin,View):
    model = QuickQuotation

    def post(self, request, *args, **kwargs):
        objs = QuickQuotation.objects.filter()
        for obj in objs:
            obj.delete()

        return JsonResponse({'message': 'Removed successfully!','status':'success'}, status=200)
    
class QuickQuotationsTemplateView(LoginRequiredMixin,View):
    def get(self, request, *args, **kwargs):
        file_path = os.path.join(settings.BASE_DIR, "static", "files", "quick_quotations-template.xlsx")
        
        if not os.path.exists(file_path):
            return JsonResponse({'message': 'File not found!','status':'error'}, status=404)

        return FileResponse(open(file_path, 'rb'))
    
class ImportQuickQuotationsView(LoginRequiredMixin,View):
    def post(self, request, *args, **kwargs):
        data = json.loads(request.POST.get('data', '{}'))
        file = request.FILES.get('file')
        
        importer = BaseImporter(user_id=request.user.id, app="quotations", model_name="QuickQuotation", file=file)

        if importer.validate_file() != 200:
            return JsonResponse(importer.validate_file(), status=400)

        send_alert({"message":"Items importing on background...",'status':'success'},room=f"private_{request.user.id}")

        df_json = importer.read_file()
        if isinstance(importer.read_file(), dict):
            return JsonResponse(df_json, status=400)
            
        importer.start_import(df_json)

        return HttpResponse(status=200)